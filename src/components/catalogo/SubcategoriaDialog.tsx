...
interface Props {
  open: boolean;
  onClose: () => void;
  onSave: (sub: CatalogSubcategoria) => void;
  editItem?: CatalogSubcategoria | null;
  categoriaId: string;
}

export function SubcategoriaDialog({ open, onClose, onSave, editItem, categoriaId }: Props) {
  ...
  const handleSave = () => {
    onSave({
      ...
      categoriaId: editItem?.categoriaId || categoriaId,
    });
    onClose();
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{editItem ? "Editar Subcategoria" : "Nova Subcategoria"}</DialogTitle>
        </DialogHeader>
